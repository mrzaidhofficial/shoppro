...
router.get('/dashboard', isAdmin, async (req, res) => {
    try {
        ...
        const revenueResult = await Order.aggregate([
            ...
            { $group: { _id: null, totalProfit: { $sum: { $multiply: ['$items.quantity', { $ifNull: ['$productData.profit', 0] }] } }, avgOrder: { $avg: '$total' } } }
        ]);
        ...
        res.render('admin/dashboard', { title: 'Admin Dashboard', totalProducts, totalOrders, totalCustomers, totalRegisteredUsers, conversionRate, totalSales, totalRevenue, avgOrderValue, recentOrders, monthlySales, topProducts, ordersByStatus });
    } catch (error) { console.error(error); res.status(500).render('error', { message: 'Error loading dashboard' }); }
});
...
router.get('/products/add', isAdmin, (req, res) => { res.render('admin/add-product', { title: 'Add Product' }); });
